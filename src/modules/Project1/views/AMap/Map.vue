  ...
  <div class="mymapM">
    ...
    <div class="search-box">
      <div class="search-postion">
        <span class="buts" @click="toBack">返回</span>
        <input type="text" placeholder="输入关键字搜索" v-model="search_key"  @input="changeInputVal(keySearchFn,300)" />
        <span class="clear" v-if="search_key" @click="search_key ='' ">
          <van-icon color="#8f8f8f" name="clear" />
        </span>
        <span class="buts border_but" @click="keySearchFn()">捜索</span>
      </div>
    </div>
...
export default {
  data() {
    return {
      center: [108.377183, 22.839739], //经度+纬度
      search_key: "", //搜索值
      lists: [], //地点列表
      search_list: [], //搜索结果列表
      marker: "",
  ...
  mounted() {
    ...
    script.src = 'https://webapi.amap.com/maps?v=1.4.13&key=306f42e565027cf5bd169459c93d6917&plugin=AMap.Geocoder'
  ...
  methods: {
    adMap() {
      ...
      // alert("this.center中心坐标==" + this.center);
      //创建标记
